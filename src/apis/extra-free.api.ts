import type { CreateExtraFeePayload, ExtraFeeResponse, ExtraFeeTypePaginationParams, ExtraFeeTypePaginationResponse, ExtraFeeTypeResponse, ExtraFeeTypeDetailResponse } from "@/types/extra-fee.type"
...
export const extraFeeTypeApi = {
    ...
    create: async (name: string, description: string) =>
        await http.post<ExtraFeeTypeDetailResponse>(EXTRA_FEE_TYPE_URL.BASE, { name, description }),
    update: async (id: string, name: string, description: string) =>
        await http.put<ExtraFeeTypeDetailResponse>(`${EXTRA_FEE_TYPE_URL.BASE}/${id}`, { extraFeeTypeId: id, name, description }),
    delete: async (id: string) =>
        await http.delete<ExtraFeeTypeDetailResponse>(`${EXTRA_FEE_TYPE_URL.BASE}/${id}`),
    getDetail: async (id: string) =>
        await http.get<ExtraFeeTypeDetailResponse>(`${EXTRA_FEE_TYPE_URL.BASE}/${id}`),
