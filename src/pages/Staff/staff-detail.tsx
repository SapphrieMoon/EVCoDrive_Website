...
export function StaffDetail({ id, open, onOpenChange }: BaseDetailProps) {
    const { data, isLoading } = staffQueries.useDetail(id as string)
                    ...
                    <div className="space-y-6 py-6 ml-4">
                        ...
                        <div className="flex flex-col items-center justify-center space-y-4 bg-muted/30 py-8 rounded-xl border border-dashed">
                            <Avatar className="h-20 w-20 border-2 border-background shadow-sm">
                                <AvatarImage src={undefined} alt={staff?.fullName} />
                            ...
                            <div className="text-center space-y-1">
                                <h3 className="font-bold text-xl leading-tight">{staff?.fullName}</h3>
                                <p className="text-sm text-muted-foreground">{staff?.email}</p>
                            </div>

                            <Badge
                                variant={staff?.isActive ? "default" : "destructive"}
                                className={cn("rounded-full px-4", staff?.isActive && "bg-green-500/10 text-green-500 border-green-500/20")}
                            >
                                <div className={cn("mr-2 h-1.5 w-1.5 rounded-full", staff?.isActive && "animate-pulse", staff?.isActive ? "bg-emerald-500" : "bg-red-500")} />
                            ...
                            <div className="rounded-lg border p-4 bg-card space-y-4">
                                <div className="flex items-start gap-3">
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <p className="text-sm font-mono break-all">{staff?.staffId}</p>
                                    </div>
                                </div>
                                ...
                                <div className="flex items-start gap-3">
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <p className="text-sm font-semibold text-primary italic">
                                            {staff?.stationName || "Chưa phân trạm"}
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="rounded-lg border p-4 bg-card space-y-4">
                                ...
                                <div className="flex justify-between items-center text-sm border-t pt-4">
                                    ...
                                    <span className="font-medium">
                                        {staff ? formatDate(staff.createdDate) : "---"}
                                    </span>
                                </div>
                                <div className="flex justify-between items-center text-sm">
                                    ...
                                    <span className="font-medium">{staff?.phone || "Chưa cập nhật"}</span>
                                </div>
                            </div>
                        </div>
                    </div>
